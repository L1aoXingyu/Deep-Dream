...
def make_step(img, model, control=None):
    ...
    for i in range(num_iterations):
        ...
        if t.cuda.is_available():
            img_variable = v(img_tensor.cuda(), requires_grad=True)
        else:
            img_variable = v(img_tensor, requires_grad=True)
...
def dream(model, base_img, octave_n=6, octave_scale=1.4):
    octaves = [base_img]
